...
class BloodPressureTracker {
    ...
    async saveReading(reading) {
        try {
            ...
            if (!response.ok) {
                ...
                const errorMessage = errorData.error || 'Failed to save reading';
    ...
    async addReading() {
        ...
        // Note: id is now auto-generated by the database (AUTO_INCREMENT)
        const reading = {
            upperPressure,
            lowerPressure,
            pulseRate,
        ...
        try {
            ...
            // Check for duplicate datetime error
            if (error.message && error.message.includes('already exists')) {
                this.showNotification('A reading with this date and time already exists', 'error');
            } else {
    ...
    async updateReading() {
        ...
        try {
            ...
            if (!response.ok) {
                const errorData = await response.json().catch(() => ({}));
                const errorMessage = errorData.error || 'Failed to update reading';
                throw new Error(errorMessage);
            }
